...
bool SLW_Text::init() {
	...
	if(!console) {
		error("can't initialize widget '%s': not placed on console",
				name);
		return false;
	}
	...
	// default text specific settings
	// wrap = true;
	scroll = true;
...
bool SLW_Text::refresh() {
	...
	for(c = 0; c < h; c++ ) {
		...
		// if wrapping, putnch returns a positive number
		// then the space left in height is decreased

		c += putnch(r->text, 0, c, r->len);
...
int SLW_Text::move_string(Row *dest, Row *src, int len) {
