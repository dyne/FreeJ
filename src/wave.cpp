...
bool WaveFile::WriteHeaderToFile(FILE* fp)
{
	...
	uint32_t wholeLength = waveHeaderLength + dataLength;
	uint32_t chunkLength = fmtChunkLength;
...
bool WaveFile::WriteSample(double sample)
{
	if (GetBitsPerChannel() == 8)
		...
		return WriteSample(/*short*/uint16_t(sample * ((1 << (16 - 1)) - 1)));
	else {
		error = "Floats can be written only as 8 or 16-bit samples";
		return false;
	}
...
bool WaveFile::WriteSample(/*short*/uint16_t sample)
{
	if (GetBitsPerChannel() != 16) {
		error = "Sample size mismatch";
		return false;
