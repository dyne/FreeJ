...
bool SdlXvScreen::init(int width, int height) {
  ...
  scr = SDL_SetVideoMode(width,height,0,SDL_HWSURFACE | SDL_ASYNCBLIT | SDL_DOUBLEBUF | SDL_HWACCEL);
  if(!scr) {
  ...
  if (yuv_overlay->pitches[0] !=
      yuv_overlay->pitches[1] * 2
      || yuv_overlay->pitches[0] !=
      yuv_overlay->pitches[2] * 2) {
